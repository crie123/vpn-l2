...
class RealInterface:
    def __init__(self, iface_hint=None, is_client=False):
        ...
        try:
            ...
            self.adapter = self.wintun.wintun.WintunCreateAdapter("vpn-l2", "vpn-l2", ctypes.byref(guid))
            if not self.adapter:
                raise WindowsError(f"Не удалось создать Wintun адаптер: {ctypes.get_last_error()}")
            
            # Start session with 1MB ring buffer
            self.session = self.wintun.wintun.WintunStartSession(self.adapter, WINTUN_MIN_RING_CAPACITY)
            if not self.session:
                self.wintun.wintun.WintunCloseAdapter(self.adapter)
                raise WindowsError(f"Не удалось запустить сессию Wintun: {ctypes.get_last_error()}")
    ...
    def _capture_packets(self):
        ...
        while self.running:
            try:
                ...
                if ctypes.windll.kernel32.WaitForSingleObject(self.read_event, 100) == 0:
                    # Read packet from Wintun
                    packet = self.wintun.wintun.WintunReceivePacket(self.session, ctypes.byref(packet_size))
                    if packet:
                        try:
                            buffer = (c_byte * packet_size.value).from_address(ctypes.cast(packet, ctypes.c_void_p).value)
                            data = bytes(buffer)
                            # Only process incoming VPN packets
                            if data and data[0] & 0xF0 == 0x40:  # IPv4 packet
                                self.packet_queue.put(data)
                        finally:
                            self.wintun.wintun.WintunReleaseReceivePacket(self.session, packet)
            except Exception as e:
    ...
    def inject(self, packet_bytes: bytes):
        try:
            ...
            if size > WINTUN_MAX_IP_PACKET_SIZE:
                raise ValueError(f"Размер пакета {size} превышает максимально допустимый {WINTUN_MAX_IP_PACKET_SIZE}")
            ...
            if packet:
                ...
                raise WindowsError(f"Не удалось выделить память для пакета: {ctypes.get_last_error()}")
        except Exception as e:
